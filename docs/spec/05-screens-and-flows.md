...
- **Each rule row carries an ignore-case control** for its *value*, off by default. It appears only for EQUALS and CONTAINS rows — a REGEX row says `(?i)` in the pattern itself, and PRESENT / ABSENT rows compare no value — so switching a row's type shows or hides the control. It is a compact per-row toggle (checkbox), not a field with a label of its own; the row stays one line.
...
- **Basic auth helper (F12):** beneath the header rules, a user id and password box build an `Authorization` header rule — the pair is base64-encoded with a colon between them — replacing any Authorization rule already present. The rule is a **REGEX** of the form `^(?i:Basic)\s+<credential>$`, so a system under test sending `BASIC`, `Basic`, or `basic` all match while the credential stays exact and a misspelled scheme (`BASSIC`) or a different one (`Bearer`) does not. The reverse is shown too: hovering a credential value — a pattern the helper built, or a plain `Basic …` value typed by hand or imported — reveals the decoded user id and password, so an encoded credential from an existing mock can be read without leaving the page.
