...
- A Linux **install script** (`install.sh`) provides one-command setup: it prompts for the admin and mock **host** ports (defaulting to 8931 / 8932 when the user presses Enter), fetches the source if not already in a checkout (git clone, or — when git is absent — a `curl`/`wget` + `tar` tarball download), writes a `.env`, and runs `docker compose up -d --build`. The README documents a copy-paste `bash <(curl …)` invocation.
